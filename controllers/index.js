...
class Controller {
  ...
  static showAddPosting(req, res) {
    ...
    Tag.findAll({})
      .then((data) => res.render("addpost", { title: "Add Post", data, errors }))
    ...
    Post.create({ title, post, img: path, UserId: req.session.loginid })
      ...
      .catch((err) => {
        if (err.name === "SequelizeValidationError") {
          res.redirect(`/posts/add?errors=` + err.errors.map((el) => el.message).join(";"));
        } else {
          res.send(err);
        }
      });
  ...
  static showUpdatePosting(req, res) {
    ...
    Post.findAll({ include: { model: Tag }, where: { id: req.params.postId } })
      .then((data) => {
        detailpost = data[0];
      ...
      .then((data) => res.render("editpost", { title: "Edit Post", detailpost, detailtag, data, errors }))
      .catch((err) => res.send(err));
  }
  ...
  static postUpdatePosting(req, res) {
    ...
    Post.update({ title, post, img, UserId: req.session.loginid }, { where: { id: +postId } })
      .then((data) => PostTag.update({ PostId: postId, TagId: tag }, { where: { PostId: postId } }))
      .then((data) => res.redirect("/posts"))
      .catch((err) => {
        if (err.name === "SequelizeValidationError") {
          res.redirect(`/posts/edit/${postId}?errors=` + err.errors.map((el) => el.message).join(";"));
        } else {
          res.send(err);
        }
      });
